...
const PORT = 3000;
// const clientDir = path.join(__dirname, '../../clients/clientB');
const outputDir = path.join(__dirname, '../../../output');

server.use(express.static(path.join(__dirname, './ui')));

// Route to list all email variations inside study folder
server.get('/output/clientB/:study', (req, res) => {
    const { study } = req.params;
    const studyPath = path.join(outputDir, 'clientB', study);
...
server.get('/output/clientB/:study/:variation', (req, res) => {
    const { study, variation } = req.params;
    const variationPath = path.join(outputDir, 'clientB', study, variation);
...
// ✅ Serve email preview
server.get('/output/clientB/:study/:variation/:language', (req, res) => {
    const { study, variation, language } = req.params;
    const outputPath = path.join(outputDir, 'clientB', study, variation, language);
