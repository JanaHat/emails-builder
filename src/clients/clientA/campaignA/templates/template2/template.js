...
export const generateEmail = ({
    ...
    const intro = ContentSection({
        ...
        children: `
            <mj-text font-size="20px" color="${headingColor}" font-weight="600">
                ${escapeHtml(greeting)}
            </mj-text>
            ${introLine ? `<mj-text font-size="16px" color="${textColor}">${escapeHtml(introLine)}</mj-text>` : ''}
            ${offerLine ? `<mj-text font-size="16px" color="${textColor}">${escapeHtml(offerLine)}</mj-text>` : ''}
            ${hasDiscount ? `
                <mj-text font-size="16px" color="${textColor}">
                    ${escapeHtml(discountLabel)} <strong>${escapeHtml(discountCode)}</strong>
                </mj-text>
            ` : `
                ${noDiscountLine ? `<mj-text font-size="16px" color="${textColor}">${escapeHtml(noDiscountLine)}</mj-text>` : ''}
            `}
        `
    });
